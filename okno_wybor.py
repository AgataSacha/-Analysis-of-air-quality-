...
class ctkAppChoice:
    def __init__(self, root: ctk.CTk, cities, df, callback: Callable):
        ...
        ctk.CTkButton(master=self.window, text="Porównanie miast", font=('Helvetica', 15),
                      command=lambda: self._close("compare")).place(relx=0.15, rely=0.7)
        ctk.CTkButton(master=self.window, text="Dane dla wybranego miasta", font=('Helvetica', 15),
                      command=lambda: self._close("city")).place(relx=0.6, rely=0.7)
